			...
			try {
				if (e.getSource() == screen.getLogin().getBtnLogIn()) {
					...
					String password = screen.getLogin().getTextFieldPassword()
							.getText();
					UserInformation currentUser = auUser.login(userName, password);

					// if the entered info is validated
					if (currentUser != null) {
						// if the UserType is Student
						...
						// else if the UserType is Adminisstrator
						else if (currentUser.getType() == 2) {
							screen.show(screen.MAINMENU);
						}// end else if
					}// end if
						// if entered info is not validated
					else {
						JOptionPane.showMessageDialog(screen,
